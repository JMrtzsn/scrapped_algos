        ...
        for asset in context.assets:
            ...
            if not LIVE:
                ...
                if asset == context.assets[0]:
                    record(price_0=asset.price, vstop_0=asset.vstop)
    ...
    if asset.vstop_trend_changed:
        ...
        if asset.vstop_is_uptrend:
            ...
            if pos_amount > 0:
                try:
                    order_target_percent(
                        asset=asset.symbol, target=0, limit_price=asset.price * 0.99
                    )
                    log.critical(
...
def trigger_liquidate_holdings(context, data):
    ...
    for coin in context.assets:
        ...
        try:
            order_target_percent(
                asset=coin.symbol, target=0, limit_price=coin_price * 0.99
            )
