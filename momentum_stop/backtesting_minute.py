...
def initialize(context):
    ...
    context.trading_pairs = ["eth_btc", "bnb_btc", 'xlm_btc', "ada_btc", "xrp_btc", "eos_btc"]
    atr_length = 3
    atr_multiplier = 4
    context.rsi_length = 14

    allocation = 1 / len(context.trading_pairs)
    context.assets = []
    ...
    for index, pair in enumerate(context.trading_pairs):
        asset = CryptoAsset(symbol(pair), atr_multiplier, atr_length, allocation)
...
class CryptoAsset(object):
    ...
    def __init__(self, symbol, atr_multiplier, atr_length, portfolio_allocation):
        self.symbol = symbol
