...
def initialize(context):
    ...
    for pair in context.target.index.unique():
...
def execution_time():
    ...
    if currentDT.hour == 18 and currentDT.minute == 1:
        return True
    else:
        return False
    ...
    try:
        ...
        if context.first_run:
            ...
            for Index, row in hist.iterrows():
                asset.atr = talib.ATR(hist.iloc[:hist.index.get_loc(Index) + 1]['high'],
...
def trigger_vstop_trend_change(context, asset):
    if asset.trend_change:
        ...
        if asset.uptrend:
            context.target.at[asset.pair, 'target'] += asset.portfolio_allocation
            context.target.loc[(context.target.index == asset.pair) & (context.target['strat'] == asset.atr_multiplier),
                               'long'] = True
            try:
                order_target_percent(asset=asset.symbol, target=context.target.at[asset.pair, 'target'][0],
                                     limit_price=asset.price * 1.01)

            except CreateOrderError as e:
                log.critical(f"CreateOrderError {e.args}")

            log.info(f"1 Day Momstop: Trend changed BUY: {asset.symbol.asset_name} Price: {asset.price}")

        elif not asset.uptrend:
            if context.portfolio.positions[asset.symbol].amount > 0 and context.target.loc[(context.target.index == asset.pair) & (context.target['strat'] == asset.atr_multiplier),'long'][0]:
                context.target.at[asset.pair, 'target'] -= asset.portfolio_allocation
                ...
                try:
                    ...
                    log.info(f"1 Day: Trend changed SOLD: {asset.symbol.asset_name} Price: {asset.price}")
                except CreateOrderError as e:
                    log.critical(f"CreateOrderError {e.args}")
        save_target(context)
...
def read_target():
    ...
    target['target'] = pd.to_numeric(target['target'], downcast='float')
    return target
...
class CryptoAsset(object):
    ...
    def update_vstop(self):
        ...
        if self.uptrend_prev:
            temp_stop = max(self.vstop, max(self.max, self.price) - self.atr_multiplier * self.atr)
        else:
            temp_stop = min(self.vstop, min(self.min, self.price) + self.atr_multiplier * self.atr)
