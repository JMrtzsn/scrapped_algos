...
def trigger_vstop_trend_change(context, asset):
    if asset.trend_change:
        ...
        if asset.uptrend:
            ...
            context.target.loc[
                (context.target.index == asset.pair)
                & (context.target["strat"] == asset.atr_multiplier),
                "long",
            ] = True
            try:
                order_target_percent(
                    asset=asset.symbol,
                    target=context.target.at[asset.pair, "target"][0],
            ...
            if (
                context.portfolio.positions[asset.symbol].amount > 0
                and context.target.loc[
                    (context.target.index == asset.pair)
                    & (context.target["strat"] == asset.atr_multiplier),
                    "long",
                ][0]
