...
def initialize(context):
    ...
    asset = CryptoAsset(symbol('btc_usdt'), dip_levels, atr_multiplier, atr_length, 1)
                ...
                if context.btc_long and coin_amount * coin_price \
                    ...
                    try:
                        order_target_percent(asset=coin.symbol, target=coin.portfolio_allocation,
                                             limit_price=coin_price * 1.01)
                    except CreateOrderError as e:
                        log.critical(f"CreateOrderError {e.args}")

                    log.critical(f"1 DAY Momstop: Bought {coin.symbol.asset_name} Confirmed")
    ...
    if asset.vstop_trend_changed:
        ...
        if asset.vstop_is_uptrend:
            if pos_amount * asset.price \
                    < (asset.portfolio_allocation * context.portfolio.portfolio_value)*0.95:
                try:
            ...
            if pos_amount > 0:
                try:
                    order_target_percent(asset=asset.symbol, target=0, limit_price=asset.price * 0.99)
...
def trigger_liquidate_holdings(context, data):
    ...
    for coin in context.assets:
        coin_price = data.current(coin.symbol, 'price')
        try:
            order_target_percent(asset=coin.symbol, target=0, limit_price=coin_price * 0.99)
...
def trigger_loss_check(context, data):
    ...
    if current_portfolio <= context.maxPortfolio - 20:
        trigger_liquidate_holdings(context, data)
        context.maxPortfolio = current_portfolio
...
if __name__ == '__main__':
    if LIVE:
        ...
        run_algorithm(
            ...
            end=pd.to_datetime('2018-8-10', utc=True),
            quote_currency='usd',
            live=False
        )
