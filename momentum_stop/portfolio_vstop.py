...
class PortfolioVstop(StrategyTemplate):
    ...
    def initialize_indicators(
        self, atr_multiplier, atr_length, rsi_length, rsi_trigger_level
    ):
    ...
    def load_indicators(self, data, asset):
        if "btc_usd" in asset.name:
            asset.rsi = talib.RSI(asset.history["price"], timeperiod=self.rsi_length)[
                -1
            ]
        ...
        if asset.vstop.vstop_trend_changed:
            ...
            if asset.vstop.vstop_is_uptrend:
                try:
                    ...
                    self.log.critical(
                        f"Trend changed BUY: {asset.symbol} Price: {asset.price}"
                    )
                except CreateOrderError as e:
                    ...
                    if "btc" in self.quote_currency:
                        if "btc_usd" in asset.name:
                            ...
                            if not context.btc_long:
                                self.log.critical(
                                    f"Sold asset {asset.symbol} requires BTC dump to USDT"
                                )
                    else:
                        order_target_percent(
                            asset=asset.symbol, target=0, limit_price=asset.price * 0.98
                        )

                    self.log.critical(
                        f"Trend changed SELL: {asset.symbol} Price: {asset.price}"
                    )
                except CreateOrderError as e:
                    self.log.critical(f"CreateOrderError {e.args}")
