...
from catalyst.api import record, order, order_target, symbol, get_order
...
def weAreLong(context, analysis):
    ...
    log.info(
        "We Are Long. Holdings: {amount} @ {cost_basis}".format(
    ...
    if context.price < Crit:
        ...
        context.level += 1
        log.info(
            "Kill Long! GO SHORT! Sold {amount} @ {price}".format(
