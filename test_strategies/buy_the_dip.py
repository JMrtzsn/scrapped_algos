...
def handle_data(context, data):
    prices1 = data.history(
        context.asset1, bar_count=context.bar_count, fields=["price"], frequency="1d"
    )
    prices2 = data.history(
        context.asset2, bar_count=context.bar_count, fields=["price"], frequency="1d"
    )
    prices3 = data.history(
        context.asset3, bar_count=context.bar_count, fields=["price"], frequency="1d"
    )
    prices4 = data.history(
        context.asset4, bar_count=context.bar_count, fields=["price"], frequency="1d"
    )
    ...
    if len(change2) >= 5:
        analysis1["ema1"] = ta.EMA(
            analysis1["changes1History"].as_matrix(), context.emaOfChanges1Length
        )
        analysis1["ema2"] = ta.EMA(
            analysis1["changes2History"].as_matrix(), context.emaOfChanges2Length
        )
    ...
    if len(change1) >= 2:
        ...
        change2 = change1.pct_change()
    if len(change2) >= 5:
        analysis2["ema1"] = ta.EMA(
            analysis2["changes1History"].as_matrix(), context.emaOfChanges1Length
        )
        analysis2["ema2"] = ta.EMA(
            analysis2["changes2History"].as_matrix(), context.emaOfChanges2Length
        )
    ...
    if len(change1) >= 2:
        ...
        change2 = change1.pct_change()
    if len(change2) >= 5:
        analysis3["ema1"] = ta.EMA(
            analysis3["changes1History"].as_matrix(), context.emaOfChanges1Length
        )
        analysis3["ema2"] = ta.EMA(
            analysis3["changes2History"].as_matrix(), context.emaOfChanges2Length
        )
    ...
    if len(change1) >= 2:
        ...
        change2 = change1.pct_change()
    if len(change2) >= 5:
        analysis4["ema1"] = ta.EMA(
            analysis4["changes1History"].as_matrix(), context.emaOfChanges1Length
        )
        analysis4["ema2"] = ta.EMA(
            analysis4["changes2History"].as_matrix(), context.emaOfChanges2Length
        )
...
def makeOrders(context, a1, a2, a3, a4):
    if (
        (getLast(a1, "ema1") < getLast(a2, "ema1"))
        and (getLast(a1, "ema1") < getLast(a4, "ema1"))
        and (getLast(a1, "ema1") < getLast(a3, "ema1"))
    ):
    ...
    if (
        (getLast(a2, "ema1") < getLast(a4, "ema1"))
        and (getLast(a2, "ema1") < getLast(a1, "ema1"))
        and (getLast(a2, "ema1") < getLast(a3, "ema1"))
    ):
    ...
    if (
        (getLast(a3, "ema1") < getLast(a2, "ema1"))
        and (getLast(a3, "ema1") < getLast(a1, "ema1"))
        and (getLast(a3, "ema1") < getLast(a4, "ema1"))
    ):
        order_target_percent(context.asset1, target=0)
        order_target_percent(context.asset2, target=0)
    ...
    if (
        (getLast(a4, "ema1") < getLast(a2, "ema1"))
        and (getLast(a4, "ema1") < getLast(a1, "ema1"))
        and (getLast(a4, "ema1") < getLast(a3, "ema1"))
    ):
        order_target_percent(context.asset1, target=0)
        order_target_percent(context.asset2, target=0)
        order_target_percent(context.asset3, target=0)
