...
def handle_data(context, data):
    ...
    if len(context.model.get_renko_prices()) == 0:
        ...
        history = data.history(
            context.asset, "price", bar_count=context.n_history, frequency=context.tf
        )
        ...
        opt_bs = opt.fminbound(
            lambda x: -evaluate_renko(brick=x, history=history, column_name="score"),
            iqr_diffs[0],
            iqr_diffs[1],
            disp=0,
        )
        ...
        # Open a position
        order_target_percent(
            context.asset, context.leverage * context.model.get_renko_directions()[-1]
        )
        ...
        last_price = data.history(
            context.asset, "price", bar_count=1, frequency="1440T",
        )
        ...
        if (
            np.sign(
                ...
                * context.model.get_renko_directions()[-1]
            )
            == -1
        ):
