...
NAMESPACE = 'binance_4hour_momstop'
...
def initialize(context):
    ...
    context.trading_pairs = ["btc_usdt",
                             "zrx_btc", 'xmr_btc', 'dash_btc', "etc_btc", 'xem_btc',
                             "bnb_btc", "ven_btc", "omg_btc", "qtum_btc", "zec_btc",
                             "ont_btc", "icx_btc", 'bcn_btc', 'zil_btc', 'lsk_btc',
                             "wan_btc", "dcr_btc", "elf_btc", "knc_btc", "eng_btc"
                             ]




    context.allocation = 1 / len(context.trading_pairs)-1
    ...
    asset = CryptoAsset(symbol('btc_usdt'), dip_levels, 2, 12, context.allocation * context.leverage)
    context.assets.append(asset)

    for index, pair in enumerate(context.trading_pairs[1:]):
        asset = CryptoAsset(symbol(pair), dip_levels, atr_multiplier, atr_length, context.allocation * context.leverage)
        context.assets.append(asset)
...
def handle_data(context, data):
    ...
    if context.day_timer % context.trading_execution_time == 0:
        ...
        for asset in context.assets[1:]:
            ...
            # ------ Order logic ------ #
            log.info(f"4 Hour Momstop: Asset: {asset.symbol.asset_name} Price: {asset.price} VSTOP: {asset.vstop}")
    ...
    if context.assets[0].symbol == asset.symbol:
        if asset.rsi < asset.dip_level:
            ...
            for coin in context.assets:
                ...
                if context.btc_long and coin_amount * coin_price \
                        < (asset.portfolio_allocation * context.portfolio.portfolio_value)*0.8:
                    try:
                    ...
                    log.critical(f"4 Hour Momstop: Bought {coin.symbol.asset_name} Confirmed")
...
def trigger_vstop_trend_change(context, data, asset):
    ...
    if asset.vstop_trend_changed:
        ...
        if asset.vstop_is_uptrend:
            ...
            if context.assets[0].symbol == asset.symbol:
                context.btc_long = True
            if context.btc_long and pos_amount * asset.price \
                    < (asset.portfolio_allocation * context.portfolio.portfolio_value)*0.8:
                try:
            ...
            log.critical(f"4 Hour Momstop: Trend changed BUY: {asset.symbol.asset_name} Price: {asset.price}")
            ...
            if pos_amount > 0:
                try:
                    ...
                    if not context.btc_long:
                        log.critical(f"Sold asset {asset.symbol.asset_name} requires BTC dump to USDT")
                    else:
                        log.critical(f"4 Hour Momstop: Trend changed SOLD: {asset.symbol.asset_name} Price: {asset.price}")
                except CreateOrderError as e:
                    log.critical(f"CreateOrderError {e.args}")
...
def trigger_liquidate_holdings(context, data):
    ...
    for coin in context.assets:
        # if this is btc_usdt skip
        # if this is BTCUSDT skip and set long
        if context.assets[0].symbol == coin.symbol:
            context.btc_long = False
        coin_price = data.current(coin.symbol, 'price')
        try:
            order_target_percent(asset=coin.symbol, target=0, limit_price=coin_price * 0.9)
            log.critical(f"4 Hour Momstop: Sold {coin.symbol.asset_name}")
        except CreateOrderError as e:
            log.critical(f"CreateOrderError {e.args}")


def trigger_buy_holdings(context, data):
    log.critical(f"Buy all triggered")
    for coin in context.assets:
        # if this is BTCUSDT skip and set to long
        if context.assets[0].symbol == coin.symbol:
            context.btc_long = True
        ...
        try:
            ...
            if context.portfolio.positions[coin.symbol].amount * coin_price \
                    < coin.portfolio_allocation * context.portfolio.portfolio_value:
                order_target_percent(asset=coin.symbol, target=coin.portfolio_allocation,
                                     limit_price=coin_price * 1.1)
                log.critical(f"4 Hour Momstop: Bought {coin.symbol.asset_name}")
                coin.vstop_set_uptrend()
        except CreateOrderError as e:
            log.critical(f"CreateOrderError {e.args}")
