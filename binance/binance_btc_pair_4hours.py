...
def initialize(context):
    ...
    asset = CryptoAsset(
        symbol("btc_usdt"), dip_levels, 2, 12, context.allocation * context.leverage
    )
    ...
    for index, pair in enumerate(context.trading_pairs[1:]):
        asset = CryptoAsset(
            ...
            atr_multiplier,
            atr_length,
            context.allocation * context.leverage,
        )
        context.assets.append(asset)
            ...
            for coin in context.assets:
                ...
                if (
                    ...
                    * 0.8
                    ...
                    log.critical(
                        f"4 Hour Momstop: Bought {coin.symbol.asset_name} Confirmed"
                    )
...
def trigger_vstop_trend_change(context, data, asset):
    ...
    if asset.vstop_trend_changed:
        ...
        if asset.vstop_is_uptrend:
            ...
            log.critical(
                f"4 Hour Momstop: Trend changed BUY: {asset.symbol.asset_name} Price: {asset.price}"
            )
            ...
            if pos_amount > 0:
                try:
                    ...
                    if not context.btc_long:
                        log.critical(
                            f"Sold asset {asset.symbol.asset_name} requires BTC dump to USDT"
                        )
                    else:
                        log.critical(
                            f"4 Hour Momstop: Trend changed SOLD: {asset.symbol.asset_name} Price: {asset.price}"
                        )
                except CreateOrderError as e:
                    log.critical(f"CreateOrderError {e.args}")
